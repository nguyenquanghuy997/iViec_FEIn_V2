...
const TABLE_ROW = [
  { id: "index", size: 74 },
  { id: "JobTypeCode", size: 166 },
  { id: "Description", size: 230 },
  { id: "CreatedUser", size: 230 },
  { id: "ModifiedUser", size: 230 },
  { id: "ModifiedDate", size: 170 },
  { id: "Status", size: 138 },
  { id: "action", size: 153 },
];

export const JobTypeItem = ({ data, order, onRefreshData }) => {
  const { JobTypeId, ModifiedDate, Status } = data || {};
  ...
  const [updateStatus] = useUpdateStatusJobTypeMutation();
  ...
  const onChangeActive = async () => {
    ...
    if (res.Succeeded) {
      refActive.current = isActive;
      enqueueSnackbar("Cập nhật trạng thái thành công", {
        variant: "success",
      });
    } else {
      methods.setValue("isActive", refActive.current);
      enqueueSnackbar(
        res.Errors?.[0]?.Description || "Xảy ra lỗi, vui lòng thử lại sau",
        { variant: "error" }
      );
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    setShowDelete(false);
    await deleteJobType({ Id: JobTypeId }).unwrap();
    onRefreshData();
  };

  const renderModifiedDate = () => {
    if (!ModifiedDate) return "-";
    return moment(ModifiedDate).format("DD/MM/YYYY HH:mm");
  };
  ...
  useEffect(() => {
    if (isActive === refActive.current) return;
    onChangeActive();
  }, [isActive]);
