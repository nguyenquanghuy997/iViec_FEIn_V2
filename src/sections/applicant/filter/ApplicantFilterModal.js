...
function ApplicantFilterModal({columns, isOpen, onClose, onSubmit}) {
    ...
    const watchProvinceId = useWatch({ name: "livingAddressProvinceIds", control });
    ...
    useEffect(() => {
            if (checked) {
                // select
                !isEmpty(watchOrganizationIds) ? handleSetDataFilter({
                    key: 'organizationIds',
                    value: watchOrganizationIds
                }) : handleSetDataFilter({key: 'organizationIds', value: []});
                !isEmpty(watchRecruitment) ? handleSetDataFilter({
                    key: 'recruitmentIds',
                    value: watchRecruitment
                }) : handleSetDataFilter({key: 'recruitmentIds', value: []});
                !isEmpty(watchPipeLine) ? handleSetDataFilter({
                    key: 'recruitmentPipelineStates',
                    value: watchPipeLine?.map((pipe) => Number(pipe))
                }) : handleSetDataFilter({key: 'recruitmentPipelineStates', value: []});
                !isEmpty(watchJobSource) ? handleSetDataFilter({
                    key: 'jobSourceIds',
                    value: watchJobSource
                }) : handleSetDataFilter({key: 'jobSourceIds', value: []});
                !isEmpty(watchOwnerIds) ? handleSetDataFilter({
                    key: 'ownerIds',
                    value: watchOwnerIds
                }) : handleSetDataFilter({key: 'ownerIds', value: []});
                !isEmpty(watchCreatorIds) ? handleSetDataFilter({
                    key: 'creatorIds',
                    value: watchCreatorIds
                }) : handleSetDataFilter({key: 'creatorIds', value: []});
                !isEmpty(watchCouncilIds) ? handleSetDataFilter({
                    key: 'councilIds',
                    value: watchCouncilIds
                }) : handleSetDataFilter({key: 'councilIds', value: []});
                // date
                watchCreatedTimeFrom ? handleSetDataFilter({
                    key: 'createdTimeFrom',
                    value: new Date(watchCreatedTimeFrom).toISOString()
                }) : handleSetDataFilter({key: 'createdTimeFrom', value: null});
                watchCreatedTimeTo ? handleSetDataFilter({
                    key: 'createdTimeTo',
                    value: new Date(watchCreatedTimeTo).toISOString()
                }) : handleSetDataFilter({key: 'createdTimeTo', value: null});
                // other
                !isEmpty(watchExpectWorkingAddressProvinceIds) ? handleSetDataFilter({
                    key: 'expectWorkingAddressProvinceIds',
                    value: watchExpectWorkingAddressProvinceIds
                }) : handleSetDataFilter({key: 'expectWorkingAddressProvinceIds', value: []});
                !isEmpty(watchApplicantSkillIds) ? handleSetDataFilter({
                    key: 'applicantSkillIds',
                    value: watchApplicantSkillIds
                }) : handleSetDataFilter({key: 'applicantSkillIds', value: []});
                !isEmpty(watchYearsOfExperience) ? handleSetDataFilter({
                    key: 'yearsOfExperience',
                    value: [Number(watchYearsOfExperience)]
                }) : handleSetDataFilter({key: 'yearsOfExperience', value: ""});
                !isEmpty(watchMaritalStatuses) ? handleSetDataFilter({
                    key: 'maritalStatuses',
                    value: [Number(watchMaritalStatuses)]
                }) : handleSetDataFilter({key: 'maritalStatuses', value: ""});
                !isEmpty(watchSexs) ? handleSetDataFilter({
                    key: 'sexs',
                    value: [Number(watchSexs)]
                }) : handleSetDataFilter({key: 'sexs', value: ""});
                // address
                !isEmpty(watchlivingAddressProvinceIds) ? handleSetDataFilter({
                    key: 'livingAddressProvinceIds',
                    value: [watchlivingAddressProvinceIds]
                }) : handleSetDataFilter({key: 'livingAddressProvinceIds', value: ""});
                !isEmpty(watchlivingAddressDistrictIds) ? handleSetDataFilter({
                    key: 'livingAddressDistrictIds',
                    value: [watchlivingAddressDistrictIds]
                }) : handleSetDataFilter({key: 'livingAddressDistrictIds', value: ""});
                !isEmpty(watchhomeTowerProvinceIds) ? handleSetDataFilter({
                    key: 'homeTowerProvinceIds',
                    value: [watchhomeTowerProvinceIds]
                }) : handleSetDataFilter({key: 'homeTowerProvinceIds', value: ""});
                !isEmpty(watchhomeTowerDistrictIds) ? handleSetDataFilter({
                    key: 'homeTowerDistrictIds',
                    value: [watchhomeTowerDistrictIds]
                }) : handleSetDataFilter({key: 'homeTowerDistrictIds', value: ""});
                // input
                watchHeightFromDebounce ? handleSetDataFilter({
                    key: 'heightFrom',
                    value: Number(watchHeightFromDebounce)
                }) : handleSetDataFilter({key: 'heightFrom', value: ""});
                watchHeightToDebounce ? handleSetDataFilter({
                    key: 'heightTo',
                    value: Number(watchHeightToDebounce)
                }) : handleSetDataFilter({key: 'heightTo', value: ""});
                watchWeightFromDebounce ? handleSetDataFilter({
                    key: 'weightFrom',
                    value: Number(watchWeightFromDebounce)
                }) : handleSetDataFilter({key: 'weightFrom', value: ""});
                watchWeightToDebounce ? handleSetDataFilter({
                    key: 'weightTo',
                    value: Number(watchWeightToDebounce)
                }) : handleSetDataFilter({key: 'weightTo', value: ""});
                watchEducationsToDebounce ? handleSetDataFilter({
                    key: 'educations',
                    value: watchEducationsToDebounce
                }) : handleSetDataFilter({key: 'educations', value: ""});
                watchExperienceToDebounce ? handleSetDataFilter({
                    key: 'experience',
                    value: watchExperienceToDebounce
                }) : handleSetDataFilter({key: 'experience', value: ""});
                watchExpectSalaryFromDebounce ? handleSetDataFilter({
                    key: 'expectSalaryFrom',
                    value: watchExpectSalaryFromDebounce
                }) : handleSetDataFilter({key: 'expectSalaryFrom', value: ""});
                watchExpectSalaryToDebounce ? handleSetDataFilter({
                    key: 'expectSalaryTo',
                    value: watchExpectSalaryToDebounce
                }) : handleSetDataFilter({key: 'expectSalaryTo', value: ""});
            }
        }, [
            checked,
