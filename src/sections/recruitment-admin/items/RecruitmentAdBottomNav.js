...
import { useGetRecruitmentAdByIdQuery } from "../RecruitmentAdSlice";
import RecruitmentAdConfirmMultipleModal from "../modals/RecruitmentAdConfirmMultipleModal";

const RecruitmentAdBottomNav = ({ selectedList, open, onClose, 
  // onOpenForm 
}) => {
  const { data: organization } = useGetRecruitmentAdByIdQuery(
    {
      Id: selectedList[0],
    },
      ...
      <Content sx={{padding:'20px 24px'}}>
        ...
        >
            ...
            {selectedList.length === 1 && (
              <>
                ...
                <ButtonGray
                  ...
                  onClick={() => handleShowConfirmMultiple("CloseRecruitmentAd")}
                  icon={
                    <Iconify
          ...
          <Box sx={{ display: "flex", alignItems: "center" }}>
            <Typography sx={{fontSize:14, fontWeight:600}}>Đã chọn: {selectedList.length}</Typography>
            <Divider
