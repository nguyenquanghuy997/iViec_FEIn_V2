...
import {AlertIcon, CheckedSwitchIcon, UnCheckedSwitchIcon} from "@/sections/organization/component/Icon";
                ...
                <>
                  <Stack flexDirection="row" alignItems="center">
                    <MuiButton
                        ...
                        sx={{fontWeight: 550, marginRight: 1, height: 36, "&:hover": {boxShadow: 'none'}}}
                    />
                    <MuiButton
        ...
        {(openActive || toggleActive) && <ConfirmModal
            ...
            subtitle={
              selected.length > 1 ?
                  (
                      <>Bạn có chắc chắn
                        muốn {selected.every(item => item.isActive === true) ? ' tắt hoạt động' : ' bật hoạt động'} {selected.length} người
                        dùng?</>
                  ) : (
                      <>{selected[0]?.isActive ? <>Bạn có chắc chắn muốn tắt hoạt động cho người
                        dùng<span>{item.lastName || selected[0]?.lastName}</span></> : <>Bạn có chắc chắn muốn bật hoạt động
                        cho người dùng<span>{item.lastName || selected[0]?.lastName}</span></>}</>
                  )
            }
            btnCancelProps={{
              title: 'Huỷ',
