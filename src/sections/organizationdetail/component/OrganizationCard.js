...
const OrganizationCard = ({
  ...
  const checkPermission = (listCanBeCustomized = [], arr = []) => {
    return (
      arr.filter((a) => listCanBeCustomized.some((c) => a.id === c.id)).length >
      0
    );
  };
  ...
  return (
    <CardUserStyle className="card-user-item" expanded={expanded}>
      <BoxFlex alignItems={"flex-start"}>
        <CardUserFormItemTitleStyle className="card-user-item-title">
          {isCheckbox &&
            ...
            checkPermission(userInGroupCanBeCustomized, item.organizations) && (
              <Checkbox
                value={item}
                checked={checked}
      ...
      <BoxFlex>
        <BoxFlex>
          <CardUserFormItemContentStyle
            className="card-user-item-content-text"
            sx={{ mr: 1 }}
          >
        ...
        <CardContent sx={{ padding: "unset !important" }}>
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center" }}>
              ...
              >
                Được mời bởi: {item?.referenceUser?.Email} ngày{" "}
                {fDate(item?.registrationTime)}
              </Typography>
              <Box
                sx={{
                  padding: "5px 8px",
