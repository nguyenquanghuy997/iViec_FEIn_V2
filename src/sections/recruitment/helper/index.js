...
export const renderRemainDay = (endDate) => {
  ...
  return (
    <>
      Còn <strong>{diffDay}</strong> ngày để ứng tuyển
    </>
  );
};
...
export const getWorkingFormNames = (aryWorkingForms = []) => {
  return aryWorkingForms
    .map((wf) => getWorkingFormName(wf.workingForm))
    .join(", ");
};
...
export const setValueFieldScan = (setValue, objectScan) => {
  ...
  if (objectScan) {
    ...
    setValue(
      "yearOfExperience",
      objectScan.yearOfExperience
        ? getYearOfExperience(objectScan.yearOfExperience)
        : ""
    );
    setValue("sex", objectScan.sex && objectScan.sex === "Nữ" ? 1 : 0);
    setValue(
      "portraitImage",
      "data:image/jpg;base64," + objectScan.portraitImage
    );
  }
};
