      ...
      <Box sx={{ px: 4, py: 3 }}>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <RHFTextField
            ...
            maxLength={255}
            disabled={
              !query?.type?.includes("copy") &&
              (recruitment?.processStatus === RECRUITMENT_STATUS.RECRUITING ||
                recruitment?.processStatus === RECRUITMENT_STATUS.CALENDARED)
            }
          />
        </Box>
        {/* Khu vực đăng tin */}
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <RHFSelect
            options={ListJobCategory?.map((item) => ({
              value: item?.id,
              label: item?.name,
            }))}
