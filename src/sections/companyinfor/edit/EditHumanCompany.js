...
const EditHumanCompany = ({onClose}) => {
    ...
    const {setError, setValue, register, handleSubmit, control, formState: {errors, isSubmitting, isValid},} = methods;
    const {fields, append, remove} = useFieldArray({control, name: "organizationHumans",});
    ...
    const handleSetDataForm = (items) => {
        setValue("organizationHumans", [...items]);
    }

    const renderDraggableItem = (item, index) => {
        return (
            <Box sx={{mx: 3, my: 3, px: 3, py: 2, background: "#F2F4F5", display: "flex", borderRadius: "4px"}} key={item.id}>
