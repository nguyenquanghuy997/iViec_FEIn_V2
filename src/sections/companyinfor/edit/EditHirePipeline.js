...
const EditHirePipeline = ({ data, onClose }) => {
  ...
  const [updatePipeline, { isLoading: isSubmitting }] =
    useUpdateCompanyPipelineMutation();

  useEffect(() => {
    if (!data) return;
    const organizationProfilePipelines = get(
      data,
    ...
    if (isEmpty(organizationProfilePipelines)) {
      ...
      setPipeLineState(
        LIST_PIPELINE.reduce((acc, curr) => {
          const match = organizationProfilePipelines?.find(
            (item) => item.type === curr.organizationProfilePipelineType
          );
          if (match) {
            acc.push({
              ...match,
  ...
  const addPipeline = (type) => {
    ...
    const newPipeLine = pipeLineState.map((item) =>
      item.organizationProfilePipelineType === type
        ? {
            ...item,
            ...pipeLine,
            isOpen: true,
  ...
  const handleRemovePipeLine = (index) => {
    const newPipeLine = pipeLineState.map((item) =>
      item.organizationProfilePipelineType === index
        ? {
            ...item,
            isOpen: false,
          }
  ...
  const prevIsValid = () => {
    const someEmpty = pipeLineState
      .filter((item) => item.isFixed || item.isOpen)
      .some((pipeLine) => pipeLine.description === "");
    if (someEmpty) {
  ...
  const onSubmit = async (e) => {
    ...
    const dataVisiblePipeLine = pipeLineState.filter(
      (item) => item.isFixed || item.isOpen
    );
    const res = {
    ...
    try {
      ...
      enqueueSnackbar(
        "Chỉnh sửa thông tin Quy trình tuyển dụng không thành công!",
        {
  ...
  return (
    <form onSubmit={onSubmit}>
      <FormModalHead
        title={"Chỉnh sửa Quy trình tuyển dụng"}
        onClose={onClose}
      />
            ...
            if (pipeline.organizationProfilePipelineType !== 2) {
              return (
                <View
                  ...
                  mb={24}
                  p={24}
                  style={{ background: style.BG_GRAY }}
                >
                  <div
                  ...
                  >
                    ...
                    >
                      <View contentcenter size={80} mr={16}>
                        {renderIconByPipelineType(
                          pipeline?.organizationProfilePipelineType
                        )}
                      </View>
                      <Typography
                        sx={{
                  ...
                  {pipeline.error?.description && (
                    <HelperText errorText={pipeline.error?.description} />
                  )}
                </View>
              );
            } else {
