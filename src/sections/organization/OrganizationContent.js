...
const OrganizationContent = () => {
  ...
  const canApproveUser = useMemo(
    () => canAccess(PERMISSIONS.APPR_USER_INVITE),
    []
  );
  ...
  const treeData = useMemo(() => {
    const loopFilterTree = (tree = [], query = "") => {
      return tree.filter((node) => {
        ...
        let isMatching =
          valueNameToEng?.indexOf(valueQueryToEng) > -1 ||
          valueCodeToEng?.indexOf(valueQueryToEng) > -1;
    ...
    return loopFilterTree(
      convertFlatDataToTree(ListOrganization)?.[0]?.children,
      valueSearch
    );
    ...
    >
      ...
      >
        <Stack flexDirection="row" alignItems="center">
          <Avatar
            alt="Logo Company"
            src={`${DOMAIN_SERVER_API}/Image/GetImage?imagePath=${
              ListOrganization?.find(
      ...
      {showMultipleDelete && (
        <OrganizationConfirmMultipleModal
