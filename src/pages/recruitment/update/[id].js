    ...
    const FormValidate = Yup.object().shape({
        ...
        minSalary: Yup.number()
            ...
            .when("currencyUnit", (currencyUnit, schema) => {
                if (currencyUnit === 0)
                    return schema.min(1000000, 'Mức lương tối thiểu ít nhất 7 chữ số').required("Mức lương tối thiểu không được bỏ trống")
                if (currencyUnit === 1)
                    return schema.min(100, 'Mức lương tối thiểu ít nhất 3 chữ số').required("Mức lương tối thiểu không được bỏ trống")
                return schema
            })
            .when("salaryDisplayType", (salaryDisplayType, schema) => {
                if (salaryDisplayType === 2)
